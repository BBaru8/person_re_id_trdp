...
if exist(fopts.featureFile,'file') ~= 0
    ...
    % parse all images 
    [ imgs,camID,personID,strs] = parsingDataset( dopts,partition );
    % compute feature
    if strcmp(fopts.featureType,'ldfv') % extract ldfv feature based on train/test split       
        for s = 1:numel(partition)
            ...
            [features{s},fopts] = ComputeFeatures(imgs,fopts,strs);
        end
    else
        [features,fopts] = ComputeFeatures(imgs,fopts,strs);
    end     
    save(fopts.featureFile, 'features','camID','personID','fopts','-v7.3');%'imgs',
end
