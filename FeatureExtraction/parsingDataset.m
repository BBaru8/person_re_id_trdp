function [ imgs,camID,personID,strs] = parsingDataset( dopts,partition )
...
switch lower(dopts.name)
            ...
            for p = 1:num_person
                ...
                for i = 1:numel(iminfo)
                    imgs{cnt} = imread(fullfile(srcdir,cam_sets{c},person_sets{p},...
                        iminfo(i).name));
                    strs{cnt} = fullfile(srcdir,cam_sets{c},person_sets{p},...
                        iminfo(i).name);
    ...
    case 'msmt17'
        ...
        cd(tmp_path);
    case 'dukemtmc'        
        %folder = fullfile(dopts.datafolder, 'DukeMTMC4ReID','ReID'); %OLD
        folder = fullfile(dopts.datafolder, 'DukeMTMC4ReID','train');
