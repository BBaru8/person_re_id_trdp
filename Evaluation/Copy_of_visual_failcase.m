...
%             pID_worst(p),camID(probeID(p))));
  imwrite(imresize(im_probe,[128,48]),sprintf('%s_r%d_p%d_c%d_Probe.png',dataset,rank_worst(p),...
            pID_worst(p),camID(probeID(p))));
...
%                     pID_worst(p),ucamID(u),f));
                imwrite(imresize(tmpim{f},[128,48]),sprintf('%s_r%d_p%d_c%d_f%d.png',dataset,rank_worst(p),...
                    pID_worst(p),ucamID(u),f));
            end
        end
    else
