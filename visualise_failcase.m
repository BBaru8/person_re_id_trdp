...
single = 1;
dataset = 'VIPeR';
savefolder = 'VIPeR'; %'hardeg'

% num_w = 30; % worst ones in each partition
num_show = 1264; % num of most common ones to show in the end

% load result
resfolder = './Results';
info = dir(sprintf('%s/rank_%s_%s_%s_*2019*.mat',resfolder,dataset,algo,feat));
if numel(info) > 1
    disp(numel(info))
...
for s = 1:numel(partition)
    ...
    for pr = 1:size(idx_probe,1)
        ...
        res_match = metric.ResMatch{s,pr};
...
%             pID_worst(p),camID(probeID(p))));
  imwrite(imresize(im_probe,[128,48]),sprintf('./Visual_FailCasesImagaes/%s_r%d_p%d_c%d_Probe.png',dataset,rank_worst(p),...
            pID_worst(p),camID(probeID(p))));

    else
        for i = 1:floor(numel(info)/f_show):numel(info)
            tmpim = imread(fullfile(folder_p,info(i).name));
            imwrite(imresize(tmpim,[128 48]),sprintf('./Visual_FailCasesImagaes/%s/%s_p%d_c%d_Probe_f%d_mr%d.png',savefolder,dataset,pID_worst(p),...
...
%                     pID_worst(p),ucamID(u),f));
                imwrite(imresize(tmpim{f},[128,48]),sprintf('./Visual_FailCasesImagaes/%s_r%d_p%d_c%d_f%d.png',dataset,rank_worst(p),...
                    pID_worst(p),ucamID(u),f));
            end
        end
    else
        ...
        for i = 1:floor(numel(info)/f_show):numel(info)
            ...
            imwrite(imresize(tmpim,[128 48]),sprintf('./Visual_FailCasesImagaes/%s/%s_p%d_c%d_f%d_mr%d.png',savefolder,dataset,pID_worst(p),...
